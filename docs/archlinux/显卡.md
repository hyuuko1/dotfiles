...
编辑 `/etc/mkinitcpio.conf`，在 HOOKS 一行删除 `kms` 并保存，然后执行 `mkinitcpio -P` 重新生成一次镜像。这能防止 initramfs 包含 nouveau 模块，避免 在 early boot 时 nouveau 和官方驱动的冲突。

**重启后验证**

```bash
# DRM 是否自动启用。用 wayland 时必须启用
$ sudo cat /sys/module/nvidia_drm/parameters/modeset
Y
$ sudo cat /sys/module/nvidia_drm/parameters/fbdev
Y
# nvidia-suspend.service nvidia-hibernate.service nvidia-resume.service 这三个 service 会被默认启用
```

用内核启动参数给 nvidia 模块配置参数 nvidia.NVreg_EnableGpuFirmware=0
...
systemctl suspend 测试一下挂起，也就是睡眠。注意不要用 systemctl sleep，这个好像是睡眠和休眠的混合体？
