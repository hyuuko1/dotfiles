...
- 开机和关机
  - 登录屏幕（SDDM）
    - 主题选择`Layan`。建议用 https://store.kde.org/p/1325235 里的 01-Layan.tar.gz 适配了 plasma6。建议先删除旧版本的，再安装新版本的。
      感觉 Win11OS-dark!!! 容易导致进桌面后卡死？ https://bugs.kde.org/show_bug.cgi?id=494804
    - 行为设置。自动登录、注销后立即登录
- 鼠标。指针速度和滚动速度都调高一点
- 虚拟键盘。Fctix5 wayland 启动器
- 显示和监视器。
  - 缩放率 125%
  - 自适应垂直同步：`永不`。如果开启，会导致外接显示器每隔十几分钟就黑屏几秒。
- 在桌面上右键，配置桌面
...
安装 kdialog 就可以使用 Dolphin 了，不需要设置任何环境变量。
进一步配置：
